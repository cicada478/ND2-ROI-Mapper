...
- 比例尺默认位于右下角，白色、10 µm；右键可选择长度、颜色、线宽、文字大小，
  或完全隐藏比例尺文字。
- ROI 文件名与拍摄参数标签由程序自动避让；位置固定，避免误拖动后混淆来源视野。
...
v1.1.2 GitHub Release 不提供 Windows Setup EXE，提供以下压缩包：

### Windows Portable

下载 `ND2-ROI-Mapper-Windows-Portable-v1.1.2.zip`，完整解压后双击其中的：
...
- Apple Silicon（M1/M2/M3/M4 等）：`ND2-ROI-Mapper-macOS-Apple-Silicon-v1.1.2.zip`
- Intel：`ND2-ROI-Mapper-macOS-Intel-v1.1.2.zip`
...
8. 在 Export 中选择格式。JPG 可进一步选择质量等级；PNG 为无损格式，因此质量
   控件会禁用。
9. 点击 `Export Annotated Image`，保存为 JPG 或 PNG。
...
├── installer/               # v1.0.0 的旧 Inno Setup 配置，不用于 v1.1.2
...
```powershell
.\build_release.ps1 -Version 1.1.2 -PythonVersion 3.13
```

脚本会在项目内创建隔离的 `.build-venv`，安装构建依赖，并使用 PyInstaller 生成：

```text
release/ND2-ROI-Mapper-Windows-Portable-v1.1.2.zip
```

macOS 必须在对应架构的 Mac 上原生构建：

```bash
bash ./build_macos_release.sh 1.1.2 Apple-Silicon
```
